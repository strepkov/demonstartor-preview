...
In the section 3.1 will be introduced tutorials for the very powerful tool which is used in different areas for building systems with any complexity level. In the section 3.2, is presented tutorials for the Rust programming language. In the section 3.3, is shown the theorem prover Z3 Solver from Microsoft.
...
In the picture, it emphasizes the difference in a class architecture. It helps to find feature architecture advantages from using new language(means Typescript). Moreover, you can run the given example and see the result directly in a browser, but in different tab, which is not really convenient. If you have some complex code, it is really helpful to see the source and the result simultaneously.
