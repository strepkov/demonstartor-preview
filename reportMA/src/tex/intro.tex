...
Right now the toolchain is using the server for compilation of models. In the future it would be much better to simplify the compilation process and carry out it directly on a client side, using a browser.
- using different tracks \newline
In the simulator's core the integrated track is used, which consists of different kinds of walls. It would be an improvement to separate the track from the main core, to be able to load different tracks. Even better to have external track constructor, which can build new tracks, like a configuration file for these tracks. In this configuration file, the data related to walls' positions should be stored. Then, it will be possible to construct not only the new tutorials, but new tutorials with various tracks. It increases variability of possible tasks, e.g., we can create a track with some intersections and develop an absolutely new controller. It will be able to manage the crossroad passing.
- possibility to use several controllers in one 3D environment. \newline
The simulator uses only one controller, which in charge of the car driving. Then we can have several controllers, which being executed simultaneously. It let us to create more complex tutorials with much wider variety of tasks. \newline
- standalone tutorial builder \newline
Create tutorials directly in 3D environment, then the tutorial builder will generate the configuration file, which will be used in the simulator. Now the configuration file for tutorials should be created manually. Currently, the process of creation new tutorials takes longer, because all information related to objects should be written manually in a text form.\newline
